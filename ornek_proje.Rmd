...
Q-Q grafiğini çizmek için alternatif olarak `ggpubr` paketinde `ggqqplot()` fonksiyonu da  kullanılabilir. Bu grafikteki gölgeli kısım güven aralığı olarak isimlendirilir. Eğer tüm noktalar güven aralığı içinde ise normallik varsayımı sağlanmaktadır. 
...
Yukarıdaki tabloda $p_{değeri}<0.05$ ise $H_0$ hipotezi reddedilir. 1999 yılındaki araçların ortalama yakıt tüketimlerinin normal dağılıma sahip olmadığı $(0.0000047<0.05)$ ve 2008 yılındaki araçların ortalama yakıt tüketimlerinin ise normal dağılıma sahiptir hipotezi reddedilemiştir $(0.0505>0.05)$ 
...
Ortalama yakıt tüketimine ait veriler normal varsayımını sağlamamakla birlikte t-testinin uygulamasını göstermek adına teste devam edelim. 
...
\end{align}
$$
Burada $\mu_{1999}$ ve $\mu_{2008}$ sırasıyla bir galon benzin ile gidilen ortalama mesafeleri temsil etmektedir. 
R programında t-testini yapmak için `t.test()` fonksiyonunu kullanılır.  
Bir galon benzin ile gidilen ortalama mesafeler (`mean_mpg`) test edilen değişken olmak üzere `~year` ifadesi ile bu değişken yıl bazında gruplara ayıralım. t testi ile iki grup arasında karşılaştırma yapılabileceğinden gruplama değişkeninde iki farklı seviye olmalıdır. `year` değişkeninde 1999 ve 2008 model araçlar olduğundan bu şart sağlanmaktadır. 
