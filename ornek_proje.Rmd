...
Burada $\mu_{1999}$ ve $\mu_{2008}$ yıllara göre araçların bir galon yakıt ile gittikleri ortalama mesafeleri temsil etmektedir. Bu iddianın doğruluğunu araştırmak için t-testini kullanabiliriz 
...
R programında t-testini yapmak için `t.test()` fonksiyonunu kullanılır. Bir galon benzin ile gidilen ortalama mesafeyi (`mean_mpg`) yıl bazında (`year`) gruplandırmak için `mean_mpg~year` komutu kullanılır. t testi ile iki grup arasında karşılaştırma yapılabileceğinden gruplama değişkeninde iki farklı seviye olmalıdır. `year` değişkeninde 1999 ve 2008 model araçlar olduğundan bu şart sağlanmaktadır. 
